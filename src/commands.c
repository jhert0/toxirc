...
static bool command_warn(Tox *tox, IRC *irc, int fid, char *arg);
...
struct Command commands[256] = {
    ...
    { "warn",    "Warns all channels and groupchats the bot is goind down",  true,  command_warn    },
...
static bool command_name(Tox *tox, IRC *UNUSED(irc), int fid, char *arg){
    if (!tox_is_friend_master(tox, fid)) {
        return false;
    }
...
static bool command_warn(Tox *tox, IRC *irc, int fid, char *UNUSED(arg)){
    if (!tox_is_friend_master(tox, fid)){
        return false;
    }
